...
class PhraseCut(object):
    ...
    def __init__(self, split, image_size=400, negative_prob=0, aug=None, aug_color=False, aug_crop=True,
        ...
        from ldm.data.third_party.PhraseCutDataset.utils.refvg_loader import RefVGLoader
        ...
        if remove_classes is None:
            ...
            if remove_classes[0] ***REMOVED*** 'pas5i':
            ...
            elif remove_classes[0] ***REMOVED*** 'zs':
            ...
            elif remove_classes[0] ***REMOVED*** 'aff':
        ...
        self.base_path = join(expanduser('/data8/stable-diffusion/ldm/data/third_party/PhraseCutDataset/data/VGPhraseCut_v0/images/'))
    ...
    def load_sample(self, sample_i, j):
        ...
        img = 2*img - 1 # get it to -1 to 1 range
        # img = self.normalize(img)
        
        img = img.permute(1,2,0) # put the channels in the end
    ...
    def __getitem__(self, i):
        ...
        if self.negative_prob > 0:
            if torch.rand((1,)).item() < self.negative_prob:
                ...
                new_phrase = None
                while new_phrase is None or new_phrase ***REMOVED*** phrase:
        ...
        if self.with_visual:
            ...
            if phrase in self.samples_by_phrase and len(self.samples_by_phrase[phrase]) > 1:
                ...
                from utils import blend_image_segmentation

                if self.mask in {'separate', 'text_and_separate'}:
                    # assert img.shape[1:] ***REMOVED*** img_s.shape[1:] ***REMOVED*** seg_s.shape ***REMOVED*** seg.shape[1:]
                    add_phrase = [phrase] if self.mask ***REMOVED*** 'text_and_separate' else []
            ...
            assert self.mask ***REMOVED*** 'text'
            vis_s = [phrase]
        
        seg = seg.unsqueeze(2).float() # put the channels in the end

        # data_x = (img,) + tuple(vis_s)

        if self.mask ***REMOVED*** 'text':
            return {'image': img, 'caption': vis_s[0], 'mask': seg, 'mask_batch': torch.zeros(0), 'cls_batch': i}
        else:
            return {'image': img, 'caption': vis_s[0], 'visual_query': vis_s[1], 'valid_visual_query': vis_s[2], 'mask': seg, 'mask_batch': torch.zeros(0), 'cls_batch': i}
